...
/// #907 — expand `<Noun1> has lower <Role> than <Noun2>` (and the
/// parallel `has higher / has equal or lower / has equal or higher`
/// shapes) into the two-clause form `<Noun1> has <Role> and <Noun2>
/// has <Role>`, returning the rewritten antecedent text and the list
/// of recorded comparison specs `(lhs_noun_token, lhs_role,
/// op, rhs_noun_token, rhs_role)` per match. Subscripted tokens
/// (`Task1`, `Task2`) are preserved verbatim in the synthetic
/// clauses so the existing subscript-aware join logic in
/// `compile.rs::antecedent_role_subscripts` continues to identify
/// each FT's role position correctly.
///
/// Iterates until no further comparison clause is recognised — the
/// rare case of multiple comparisons in one rule (`Task1 has lower
/// Priority than Task2 and Task1 has higher Cost than Task2`)
/// rewrites to four synthetic FT clauses + two comparison specs.
///
/// The comparator keyword vocabulary lifts to
/// `CrossAntecedentComparatorTable` in `parse_forml2_stage2.rs`,
/// which reads parallel enum-value declarations from the grammar.
/// `match_clause` enforces that both noun tokens AND the role token
/// are declared nouns (after subscript strip), so a stray
/// `<word> has lower <word> than <word>` shape that doesn't satisfy
/// the noun-catalog gate stays unrewritten and falls through to
/// the unresolved-clause path.
fn expand_cross_antecedent_comparisons(
    antecedent: &str,
    noun_names: &[String],
) -> (String, Vec<(String, String, String, String, String)>) {
    let table = crate::parse_forml2_stage2::CrossAntecedentComparatorTable::boot();
    let mut current = antecedent.to_string();
    let mut specs: Vec<(String, String, String, String, String)> = Vec::new();
    // Per-iteration: split on top-level " and " (literal-aware), try
    // to match each clause, rewrite the first matching clause + record
    // its spec. Repeat until a pass finds no match so multi-comparison
    // rules expand to fixpoint. Bounded by the number of clauses so
    // there's no risk of an infinite loop even if a match's rewritten
    // form re-matches.
    let bound = current.matches(" and ").count() + 2;
    for _ in 0..bound {
        let parts: Vec<String> = split_top_level_and(current.as_str())
            .into_iter()
            .map(|s| s.to_string())
            .collect();
        ...
        for part in parts.into_iter() {
            ...
            if let Some((lhs_token, lhs_base, op, rhs_token, rhs_base)) =
                table.match_clause(trimmed, noun_names)
            {
                // Find the comparison role from the original clause —
                // `match_clause` returns the base noun but not the
                // full role token. Re-derive it from the clause text
                // between the keyword and ` than `.
                let role_token = comparison_role_from_clause(trimmed, &table)
                    .unwrap_or_else(|| lhs_base.clone());
                let role_name = role_token.clone();
                let synthetic = alloc::format!(
                    "{} has {} and {} has {}",
                    lhs_token, role_name, rhs_token, role_name);
                new_parts.push(synthetic);
                specs.push((
                    lhs_token.to_string(),
                    role_name.clone(),
                    op.to_string(),
                ...
                // Cancel out base bindings that the compiler doesn't read.
                let _ = (lhs_base, rhs_base);
                rewrote = true;
            } else {
                new_parts.push(part);
            }
        ...
        if !rewrote { break; }
        current = new_parts.join(" and ");
    }
    (current, specs)
}

/// Extract the role token from a recognised comparison clause. The
/// table's `match_clause` returns the LHS/RHS base nouns but folds
/// the role-as-noun into the same base-stripped form. We re-walk the
/// clause text to recover the verbatim role token (which may carry
/// spaces, e.g. `Task ID`) sitting between the matched keyword and
/// ` than `.
fn comparison_role_from_clause(
    clause: &str,
    table: &crate::parse_forml2_stage2::CrossAntecedentComparatorTable,
) -> Option<String> {
    for (keyword, _op) in table.iter() {
        let needle = alloc::format!(" {} ", keyword);
        let Some(idx) = clause.find(needle.as_str()) else { continue; };
        let after = clause[idx + needle.len()..].trim();
        let than_idx = after.find(" than ")?;
        return Some(after[..than_idx].trim().to_string());
    }
    None
}

/// Look up the natural-language phrase for a comparator op so an
/// unresolved-clause diagnostic stays readable when a comparison
/// can't be placed against any antecedent (no FT carries the role,
/// or the LHS/RHS noun token didn't survive resolution).
fn cross_comparator_phrase_for_op(op: &str) -> &'static str {
    match op {
        "<"  => "has lower",
        ">"  => "has higher",
        "<=" => "has equal or lower",
...
fn resolve_derivation_rule(
    ...
    // #907 — pre-pass: expand cross-antecedent role comparison clauses
    // (`<Noun1> has lower <Role> than <Noun2>` and the parallel
    // `has higher / has equal or lower / has equal or higher` forms)
    // into two explicit FT antecedents (`<Noun1> has <Role> and
    // <Noun2> has <Role>`) plus a recorded comparison spec. The
    // expansion lets the existing antecedent_role_subscripts logic
    // (compile.rs) see two distinct subscripted FT antecedents on
    // each side, and the recorded specs land as AntecedentRoleComparison
    // entries after the main resolution loop maps each Noun token
    // back to its antecedent index. Behavior round-trips for rules
    // whose body carries no comparison clause (the common case).
    let cross_comparisons: Vec<(String, String, String, String, String)> = {
        let sep_offset = rule.text.find(" iff ")
            .map(|i| (i, i + 5))
            .or_else(|| rule.text.find(" if ").map(|i| (i, i + 4)))
            .or_else(|| rule.text.find(" when ").map(|i| (i, i + 6)));
        match sep_offset {
            Some((sep_start, sep_end)) => {
                ...
                let (rewritten, specs) = expand_cross_antecedent_comparisons(
                    &antecedent_part, &noun_names);
                if !specs.is_empty() {
                    rule.text = format!("{}{}{}",
                        consequent_part, sep_word, rewritten);
                }
                specs
            }
            None => Vec::new(),
    ...
    // #907 — map each pre-pass-recorded comparison spec to an
    // `AntecedentRoleComparison` by locating the antecedent whose
    // synthetic FT clause carries the LHS / RHS subscripted noun
    // token + the comparison role. The synthetic clauses always
    // appear in pairs (`<lhs_token> has <role> and <rhs_token> has
    // <role>`) inserted in place of the comparison clause, so they
    // exist as consecutive antecedent_parts and resolve to FTs with
    // the comparison role. We walk the parts list (already split for
    // resolution above) to recover their indices.
    //
    // Resolution failures (noun token not in antecedent_sources, or
    // comparison role not on the matched FT) skip the comparison
    // and surface the original spec under `unresolved_clauses` so
    // `check.rs` reports it — same silent-drop convention
    // `antecedent_filters` uses for un-resolvable filter clauses.
    rule.antecedent_role_comparisons.clear();
    if !cross_comparisons.is_empty() {
        let part_strs: Vec<&str> = antecedent_parts.iter().copied().collect();
        for (lhs_token, lhs_role, op, rhs_token, rhs_role) in cross_comparisons.iter() {
            // Synthetic-clause shape: `<token> has <role>`. Find the
            // antecedent index where the clause matches that shape.
            let lhs_needle = alloc::format!("{} has {}", lhs_token, lhs_role);
            let rhs_needle = alloc::format!("{} has {}", rhs_token, rhs_role);
            let lhs_idx = part_strs.iter().position(|p| p.trim() == lhs_needle);
            let rhs_idx = part_strs.iter().position(|p| p.trim() == rhs_needle);
            match (lhs_idx, rhs_idx) {
                (Some(li), Some(ri)) => {
                    // Validate that both antecedents resolved to FTs
                    // that actually carry the comparison role. Drop
                    // the comparison if either FT lacks the role
                    // (downstream compile_join_derivation would silently
                    // drop the predicate anyway).
                    let lhs_ft_ok = rule.antecedent_sources.get(li)
                        .and_then(|s| ir.fact_types.get(s.fact_type_id()))
                        .map(|ft| ft.roles.iter().any(|r| r.noun_name == *lhs_role))
                        .unwrap_or(false);
                    let rhs_ft_ok = rule.antecedent_sources.get(ri)
                        .and_then(|s| ir.fact_types.get(s.fact_type_id()))
                        .map(|ft| ft.roles.iter().any(|r| r.noun_name == *rhs_role))
                        .unwrap_or(false);
                    if lhs_ft_ok && rhs_ft_ok {
                        ...
                        rule.unresolved_clauses.push(alloc::format!(
                            "{} {} {} than {} (role {} not on antecedent FT)",
                            lhs_token,
                            cross_comparator_phrase_for_op(op),
                            lhs_role,
                            rhs_token,
                            lhs_role));
                    }
                ...
                _ => {
                    rule.unresolved_clauses.push(alloc::format!(
                        "{} {} {} than {} (antecedent_sources missing the comparison FT)",
                        lhs_token,
                        cross_comparator_phrase_for_op(op),
                        lhs_role,
                        rhs_token));
