...
pub(crate) struct CompiledDerivation {
    ...
    /// resolved at evaluation time). Used by
    /// `evaluate::forward_chain_stratified` to build the dependency
    /// graph between rules at runtime so 3+-level priority cascades
...
/// (consequent cell + role literals + negation-read cells) from a
/// `DerivationRuleDef`. Used at every `CompiledDerivation` construction
/// site so the runtime forward chainer can topologically stratify rules
/// that participate in a 3+-level priority cascade.
///
/// `consequent_cell` is the `Literal(..)` cell id, or empty for dynamic
/// shapes. `consequent_role_literals` is the (role, value) pin set
/// authored on the consequent. `negation_reads` enumerates each
/// `AbsenceOf` antecedent paired with that antecedent's role-literal
/// pins (extracted from `antecedent_role_literals` at the matching
/// antecedent index). The pin pairing is what lets the chainer
/// distinguish "rule A's consequent could satisfy rule B's AbsenceOf"
/// (creates a dependency edge) from "rule A writes the same cell but
/// with a non-overlapping role literal" (no edge — three rules all
/// writing 'Task_has_Target_Posture' with different literals don't
/// form a dependency cycle).
fn derivation_dep_metadata(rule: &crate::types::DerivationRuleDef) -> (
    String,
    Vec<(String, String)>,
...
/// task-814-stratify-3plus: convenience helper for synthetic rules
/// (subtype inheritance, SM init/event-fold, CWA negation, …) that
/// know their consequent cell but don't go through a `DerivationRuleDef`.
/// Returns the metadata triple in the same shape as
/// `derivation_dep_metadata` so all `CompiledDerivation` construction
/// sites can share field initialisation.
fn derivation_dep_metadata_synth(consequent_cell: String) -> (
    String,
    Vec<(String, String)>,
    Vec<(String, Vec<(String, String)>)>,
) {
    ...
    // created noun to gate which derivations run (O(relevant) instead of O(all)).
    {
        ...
        let subtype_pairs: Vec<(String, String)> = {
            ...
            if let Some(facts) = st_cell.as_seq() {
                for f in facts.iter() {
                    if let (Some(sub), Some(sup)) = (
                        crate::ast::binding(f, "subtype"),
                        crate::ast::binding(f, "supertype"),
                    ) {
                        pairs.push((sub.to_string(), sup.to_string()));
                    }
                }
            }
            ...
            if let Some(facts) = noun_cell_for_pairs.as_seq() {
                for f in facts.iter() {
                    if let (Some(sub), Some(sup)) = (
                        crate::ast::binding(f, "name"),
                        crate::ast::binding(f, "superType"),
                    ) {
                        let entry = (sub.to_string(), sup.to_string());
        ...
        // side of the auto-fill edge. Sourced from the
        // `Constraint` cell directly so the metamodel index doesn't
        // depend on a `data: CellIndex` rebuild — `cell_index_from_state`
        // is already invoked once by the outer `compile_to_defs_state`
        ...
        let ss_autofill_ft_pairs: Vec<(String, String)> = {
            ...
            if let Some(facts) = c_cell.as_seq() {
                for f in facts.iter() {
                    ...
                    // `cell_index_from_state` (compile.rs:2189-2210) so
                    // SpanDef.subset_autofill round-trips intact.
                    #[cfg(feature = "std-deps")]
                    if let Some(json) = crate::ast::binding(f, "json") {
                        if let Ok(cdef) = serde_json::from_str::<ConstraintDef>(json) {
                    ...
                    // No flat-fields fallback: the flat `span<i>_*`
                    // bindings strip the `subset_autofill` marker
                    // (compile.rs:991 / 2201), so without the JSON
                    // round-trip we can't tell which SS spans opt
                    // in. Skipping here is consistent with
                    // `cell_index_from_state`'s flat-fallback shape.
                }
            }
            pairs
        };
...
pub(crate) fn compile(state: &crate::ast::Object) -> CompiledModel {
    ...
    compile_data_with_state(&data, Some(state))
}

/// Core compilation: CellIndex -> CompiledModel. The `state` parameter
...
fn compile_derivations(data: &CellIndex, state_machines: &[CompiledStateMachine]) -> Vec<CompiledDerivation> {
    ...
    // CompiledDerivation that, every forward-chain round, materialized
    // the all-pairs transitive composition of every chaining binary
    // Fact Type into synthetic `_transitive_<ft1>_<ft2>` cells. It was
    // removed (task-969) as an unconsumed eager materialization: no
    // consumer ever read those cells — SM transition validation joins
    // the explicit `Transition_is_from_Status` / `_is_to_Status` /
    // `_is_defined_in_State_Machine_Definition` cells directly, never a
    // transitive closure, and the `command.rs` SM gate's
    ...
    // never contains `_transitive_`). This mirrors the removal of the
    // eager CWA-negation complement (readings/core/derivation.md): an
    // eager materialization every consumer filtered out as noise.
...
pub(crate) fn compile_explicit_derivation(data: &CellIndex, rule: &DerivationRuleDef) -> CompiledDerivation {
    ...
    let func = match antecedent_ids.len() {
        ...
        _ => {
            ...
            // (a') Existential-over-join fanout (task-814).
            //
            // Trigger: path (a) found NO join roles (the consequent-
            // role intersection across antecedents is empty), but
            // there exists a noun shared by ant0 and every other
            // antecedent that is invisible to the consequent — the
            // EXISTENTIAL join key. Source shape:
            //
            //   * X has Y 'lit' iff X concerns some Z that has Y 'lit'.
            //
            // After `expand_that_relatives`:
            //   antecedent 0: X concerns Z   (roles {X, Z})
            //   antecedent 1: Z has Y        (roles {Z, Y})
            //   consequent  : X has Y        (roles {X, Y})
            //
            // The join key here is Z. Path (a) computes
            // `{X, Y} ∩ {X, Z} ∩ {Z, Y}` = ∅ and falls through; the
            // existence-check fallback below then fires ONCE GLOBALLY
            // and projects bindings from ant0's first fact, dropping
            // every other qualifying X. Path (a') restores per-a0
            // fanout when an existential key exists, preserving the
            // role-of-consequent path's bindings/literal semantics
            // (a0 supplies X; consequent_role_literals pin Y).
            //
            // Sourceability: emit one consequent per a0 only when
            // every non-literal consequent role can be sourced from
            // a0's bindings. Otherwise the per-a0 emit would carry
            // phi for that role — strictly worse than the existence-
            // check fallback's first-fact projection. Keep falling
            // through in that case so older rule shapes don't
            // regress.
            //
            ...
            let existential_join_roles: Vec<String> = if join_roles.is_empty() {
                let ant0_roles: Vec<String> = data.fact_types.get(&antecedent_ids[0])
                    .map(|ft| ft.roles.iter().map(|r| r.noun_name.clone()).collect())
                    .unwrap_or_default();
                let cons_role_set: HashSet<String> = cons_roles_for_join.iter()
                    .cloned()
                    .collect();
                ant0_roles.iter()
            ...
            // Every non-literal consequent role must be sourceable
            // from a0 — otherwise the per-a0 emit would bind that
            // role to phi.
            let ant0_role_names: HashSet<String> = data.fact_types.get(&antecedent_ids[0])
                .map(|ft| ft.roles.iter().map(|r| r.noun_name.clone()).collect())
                .unwrap_or_default();
            let cons_roles_for_source = data.fact_types.get(&consequent_id)
                .map(|ft| ft.roles.clone())
                .unwrap_or_default();
            let lit_pinned: HashSet<String> = rule.consequent_role_literals.iter()
                .map(|crl| crl.role.clone())
                .collect();
                ...
                && has_positive_other
            {
                ...
                // Bindings come from a0 (literal pin wins per role)
                // — same as path (a). `all_cons_roles_sourceable`
                // above guarantees every non-literal consequent role
                // is a role of a0, so role_value_by_name on a0 finds
                // every binding.
                let cons_roles = data.fact_types.get(&consequent_id)
                    .map(|ft| ft.roles.clone())
                    .unwrap_or_default();
...
fn compile_join_derivation(data: &CellIndex, rule: &DerivationRuleDef) -> CompiledDerivation {
    ...
    // Build role-index lookup for each POSITIVE antecedent (the iterative
    // join only sees positives — AbsenceOf is applied as a final guard).
    ...
    //
    // task-814-join-absence: `antecedent_index` on filters/literals
    // points at the user-authored slot (including AbsenceOf positions).
    // Translate to the positive-only slot before pushing — and skip
    // filters whose target is an AbsenceOf antecedent (those literals
    // are consumed by the guard predicate below, not by the join
    // extractor).
    let original_to_positive: hashbrown::HashMap<usize, usize> = positive_idx.iter().enumerate()
        .map(|(pos, &orig)| (orig, pos))
        .collect();
    let mut preds_by_idx: Vec<Vec<Func>> = vec![Vec::new(); antecedent_ids.len()];
    for af in rule.antecedent_filters.iter() {
        let Some(&pos_idx) = original_to_positive.get(&af.antecedent_index) else { continue };
        if let Some(ft_id) = antecedent_ids.get(pos_idx) {
            if let Some(ft) = data.fact_types.get(ft_id) {
                if let Some(p) = build_antecedent_filter_pred(af, ft) {
                    preds_by_idx[pos_idx].push(p);
                }
            }
        }
    }
    for arl in rule.antecedent_role_literals.iter() {
        let Some(&pos_idx) = original_to_positive.get(&arl.antecedent_index) else { continue };
        if let Some(ft_id) = antecedent_ids.get(pos_idx) {
            if let Some(ft) = data.fact_types.get(ft_id) {
                if ft.roles.iter().any(|r| r.noun_name == arl.role) {
                    let pred = Func::compose(
                    ...
                    preds_by_idx[pos_idx].push(pred);
                }
            }
        }
    }
    ...
    //
    // task-814-join-absence: comparison indices point at user-authored
    // slots. Translate to positive-only slots and drop comparisons that
    // reference an AbsenceOf antecedent (their value isn't in the joined
    // tuple — semantically a cross-antecedent comparison against a
    // negated FT is undefined).
    let role_comp_preds: Vec<Func> = rule.antecedent_role_comparisons.iter().filter_map(|c| {
        // Both antecedent indices must point at positive antecedents and
        // fall within range; both roles must resolve on their respective
        // FTs. Drop silently otherwise (mirrors antecedent_filters'
        // silent-drop behavior).
        let lhs_pos = *original_to_positive.get(&c.lhs_antecedent_index)?;
        let rhs_pos = *original_to_positive.get(&c.rhs_antecedent_index)?;
        if lhs_pos >= n || rhs_pos >= n {
            return None;
        }
        let lhs_ri = find_role(lhs_pos, &c.lhs_role)?;
        let rhs_ri = find_role(rhs_pos, &c.rhs_role)?;
        let lhs_val = Func::compose(role_value(lhs_ri), access_fact(lhs_pos, n));
        let rhs_val = Func::compose(role_value(rhs_ri), access_fact(rhs_pos, n));
        Some(Func::compose(
...
fn sm_status_bfs_depths(sm: &CompiledStateMachine) -> HashMap<String, usize> {
    ...
    while let Some(s) = queue.pop_front() {
        let d = depth.get(&s).copied().unwrap_or(0);
        for (from, to, _) in sm.transition_table.iter() {
...
/// sm-fold-as-predicate: the ordered-left-fold branch of the SM reconstruction
/// fold. For each resource it gathers events across ALL trigger cells, orders
/// them by `occurred at Timestamp` (timeless events fall back to "" and keep
/// input order, stable), and `FoldL(sm.func)` from s0 — so the resource's
/// CURRENT status is the terminal accumulator of a genuine left fold (the LAST
/// applicable event's target). This is correct for CYCLIC (block↔unblock) and
/// COMPETING (finish vs delete) histories where the retired per-round
/// from-guarded set-application oscillated / raced.
///
/// The from-guard is now INTRINSIC to the fold: `sm.func:<status, trigger>`
/// falls through to `Selector(1)` (the current status) for any `<from, trigger>`
/// pair with no matching transition, so an event inapplicable from the current
/// status is a NO-OP in the fold — never a wipe — exactly the property the old
/// per-round guard enforced, but now order-correct.
///
/// Tuple shape per event: `<<resource>, trigger, <timestamp>>`. resource and
/// timestamp are 1-element projection SEQs (or `<>` when the role is absent),
/// NOT scalars: `Construction` is ⊥-preserving (ast.rs), so a scalar projection
/// that bottomed out on a missing role would collapse the whole tuple and drop
/// a timeless event. Carried as seqs they never bottom; `Selector(1)` unwraps
/// them to the atom at use sites, and a dirty (role-less) fact yields a non-atom
/// IndexBy key that is skipped — the same φ-tolerance the old fold had.
fn sm_ordered_fold_branch(sm: &CompiledStateMachine) -> Func {
    let sm_noun = sm.noun_name.clone();
    let sm_noun_obj = Object::atom(&sm_noun);
    let initial_obj = Object::atom(&sm.initial);

    // Per-fact projections (kept as SEQs — see the doc comment).
    let resource_proj = Func::compose(
        Func::apply_to_all(Func::Selector(2)),
        Func::filter(Func::compose(Func::Eq, Func::construction(vec![
            Func::Selector(1),
            Func::constant(sm_noun_obj.clone()),
    ...
    // Tagged event stream, Concat'd across DISTINCT trigger cells. A trigger
    // shared by several transitions (delete-from-{pending,progress,completed}
    // all fire on `Task is deleted`) is read ONCE so its events aren't
    // multiplied.
    let mut seen: Vec<String> = Vec::new();
    for (_, _, event_ft) in sm.transition_table.iter() {
        if !seen.iter().any(|e| e == event_ft) { seen.push(event_ft.clone()); }
    }
    let tagged_cells: Vec<Func> = seen.iter().map(|event_ft| {
        let cell = event_ft.replace(' ', "_");
        let tag_one = Func::construction(vec![
            resource_proj.clone(),
            Func::constant(Object::atom(event_ft)),
            ts_proj.clone(),
        ]);
        Func::compose(Func::apply_to_all(tag_one), extract_facts_from_pop(&cell))
    }).collect();
    let all_tagged = Func::compose(Func::Concat, Func::construction(tagged_cells));

    // Resource atom of a tuple: Selector(1) of the resource projection seq.
    let resource_key = Func::compose(Func::Selector(1), Func::Selector(1));
    // Drop dirty subjects BEFORE grouping: a φ-token ("φ") or empty ("")
    // subject is a real atom (so IndexBy would NOT skip it) but must not mint a
    // φ-keyed SM status — the old fold filtered these explicitly. A truly absent
    // (role-less) subject yields a non-atom key that IndexBy already skips.
    let keep_clean = Func::compose(Func::Not, Func::compose(Func::Or,
        Func::construction(vec![
            Func::compose(Func::Eq, Func::construction(vec![
                resource_key.clone(), Func::constant(Object::atom("φ"))])),
            Func::compose(Func::Eq, Func::construction(vec![
                resource_key.clone(), Func::constant(Object::atom(""))])),
        ])));
    let clean_tagged = Func::compose(Func::filter(keep_clean), all_tagged);
    let grouped = Func::compose(Func::IndexBy(Box::new(resource_key)), clean_tagged);

    // ── Synthetic transition-rank for ORDERING timeless events (the live db and
    // migration readings carry no occurred-at timestamps). rank(event) = BFS
    // depth of the event's FROM status from `initial`, with events whose target
    // is TERMINAL (no outgoing transition, e.g. `deleted`) pushed AFTER all
    // non-terminal events so a terminal event folds LAST. Real timestamps, when
    // present, dominate (the double OrderBy below); rank is the tie-break that
    // makes a timeless `started→blocked→unblocked` history fold in CAUSAL order
    ...
    for (from, to, event_ft) in sm.transition_table.iter() {
        ...
        let r = if to_terminal { max_depth + 1 + from_depth } else { from_depth };
        let slot = rank.entry(event_ft.clone()).or_insert(0);
        if r > *slot { *slot = r; }
    }
    // rank_chain: trigger atom -> rank decimal string (OrderBy parses it
    // numerically). Fallback "9999" sorts any unknown trigger last.
    let mut rank_chain = Func::constant(Object::atom("9999"));
    for event_ft in seen.iter().rev() {
        let r = rank.get(event_ft).copied().unwrap_or(9999);
        rank_chain = Func::condition(
            Func::compose(Func::Eq, Func::construction(vec![
                Func::Id, Func::constant(Object::atom(event_ft))])),
            Func::constant(Object::atom(&r.to_string())),
            rank_chain,
        );
    }
    let rank_key = Func::compose(rank_chain, Func::Selector(2));

    // Per-resource fold step: `<<status, resource>, trigger>` ->
    //   `<sm.func:<status, trigger>, resource>`.
    let step = {
        ...
        let next_status = Func::compose(sm.func.clone(),
            Func::construction(vec![status, trigger]));
        Func::construction(vec![next_status, resource])
    };

    // Order a resource's group, project to the trigger stream, fold from
    // `<initial, resource>`. STABLE double-sort: by synthetic rank FIRST (inner)
    // then by occurred-at Timestamp (outer, stable) — so real timestamps
    // dominate when present and timeless events fall back to causal rank. The ts
    // key is Selector(1) of the ts projection ("" / non-atom → sorts first,
    // before any real timestamp: historical-before-new).
    let ts_key = Func::compose(Func::Selector(1), Func::Selector(3));
    let ordered_group = Func::compose(
        Func::OrderBy(Box::new(ts_key)),
        Func::OrderBy(Box::new(rank_key)),
    );
    let ordered_triggers = Func::compose(
        Func::apply_to_all(Func::Selector(2)),
        ordered_group,
    );
    let resource_of_group = Func::compose(
        Func::Selector(1),
        Func::compose(Func::Selector(1), Func::Selector(1)),
    );
    let seed_pair = Func::construction(vec![
        Func::constant(initial_obj.clone()),
        resource_of_group,
    ]);
    let folded_pair = Func::compose(
        Func::FoldL(Box::new(step)),
        Func::construction(vec![seed_pair, ordered_triggers]),
    );

    // Emit the 3-fact shape from `<final_status, resource>`.
    let st = Func::Selector(1);
    let res = Func::Selector(2);
    let emit = Func::construction(vec![
        Func::construction(vec![
            Func::constant(Object::atom("State_Machine_is_instance_of_Noun")),
            Func::constant(Object::atom("State Machine is instance of Noun (reconstruction-fold)")),
            Func::construction(vec![
                Func::construction(vec![Func::constant(Object::atom("State Machine")), res.clone()]),
                Func::construction(vec![Func::constant(Object::atom("Noun")), Func::constant(sm_noun_obj.clone())]),
            ]),
        ]),
        Func::construction(vec![
            Func::constant(Object::atom("State_Machine_is_currently_in_Status")),
            Func::constant(Object::atom("State Machine is currently in Status (reconstruction-fold)")),
            Func::construction(vec![
                Func::construction(vec![Func::constant(Object::atom("State Machine")), res.clone()]),
                Func::construction(vec![Func::constant(Object::atom("Status")), st]),
            ]),
        ]),
        Func::construction(vec![
            Func::constant(Object::atom("State_Machine_is_for_Resource")),
            ...
            Func::construction(vec![
                Func::construction(vec![Func::constant(Object::atom("State Machine")), res.clone()]),
                Func::construction(vec![Func::constant(Object::atom("Resource")), res.clone()]),
            ]),
        ]),
    ]);
    let fold_one_resource = Func::compose(emit, folded_pair);
...
fn compile_sm_reconstruction_fold(sm: &CompiledStateMachine) -> CompiledDerivation {
    ...
    // sm-fold-as-predicate (reopened 2026-06-08): a GENUINE per-resource ordered
    // left-fold replaces the retired per-round, from-guarded SET-application,
    // which OSCILLATED on cyclic transitions (block↔unblock: each fixpoint round
    // flipped the status, so the terminal value depended on iteration parity)
    // and RACED on competing transitions (finish vs delete from the same `from`
    // both emitted, the "exactly one Status" upsert resolving nondeterministically).
    //
    // The seed (s0) is the first branch: it seeds `initial` for every instance,
    // idempotent on the status cell, and is the ONLY source of status for an
    // eventless resource (and for a 0-transition SM). The ordered-fold branch
    // (`sm_ordered_fold_branch`) reconstructs each resource's CURRENT status by
    // folding `sm.func` over its timestamp-ordered event stream — so the terminal
    // status is the LAST applicable event's target, deterministically. Concat
    // unions the two fact streams; the status cell's upsert-safety (keyed on the
    // `_sm_event_fold_` id + State Machine, see compute_upsert_safe_cells)
    // collapses seed+folded to the folded value downstream.
    let branches = vec![sm_seed_func(sm), sm_ordered_fold_branch(sm)];
    let func = Func::compose(Func::Concat, Func::construction(branches));
...
/// Test-only accessors for the two RETIRED imperative SM folds. The
/// byte-identical parity tests that drove them are gone (sm-fold-as-predicate
/// reopened: the live fold is now the ordered FoldL, which intentionally
/// diverges from the event-fold on cyclic/competing histories). Kept
/// `#[allow(dead_code)]` as the historical reference impls; full deletion is a
/// tidy-up follow-up.
#[cfg(test)]
#[allow(dead_code)]
...
///
/// task-820 (post-revision): the predicate is purely a schema-level
/// question now — "does the FT's compile-time `key_roles` cover every
/// UC span's role?" — with no runtime cell-shape gate. Pre-1.0 design
/// choice: storage IS the constraint for keyed cells. Any caller that
/// builds state via the legacy `cell_push` Seq-append path needs to
/// migrate to `cell_put_keyed`; we don't carry a Seq safety net.
fn uc_storage_covered_for_all_groups(
    ...
    span_groups.iter().all(|(ft_id, spans_in_group)| {
        ...
        // O(|key_roles|) per span; key sizes are tiny (1 for single-
        // role UCs, 2-3 for composites) so this is negligible at
        // compile time.
        spans_in_group.iter().all(|s| key_roles.contains(&s.role_index))
    })
}
...
fn compile_uniqueness_ast(
    ...
    // task-820: a UC whose spans are entirely covered by the Map-
    // storage key of every FT it touches is a structural no-op in the
    // constraint evaluator. The cell-side keyed-store
    // (`ast::cell_put_keyed`) returns `Err(KeyConflict)` the moment a
    // second tuple lands at the same scope key with a different non-
    // key value; the forward-chain emit path lifts that Err into a
    // panic (`evaluate.rs::integrate_round_facts`), and the apply
    // path will surface it as a UC violation (task-822). The cross-
    // scan `has_any_dup` Func that the legacy branches build below is
    // therefore redundant for these UCs — by construction the
    // population never contains the duplicate it's looking for.
    //
    // The early-return fires when BOTH:
    //   (a) every span's FT has `key_roles = Some(rs)`,
    //   (b) every span's `role_index` ∈ `rs`.
    //
    // No cell-shape safety net. Pre-1.0 design choice: any caller
    // building state via the legacy `cell_push` Seq-append path
    // bypasses Map storage and therefore bypasses UC enforcement —
    // the right fix is to migrate that caller to `cell_put_keyed`,
    // not to keep the cross-scan Func as a fallback.
    //
    // Cross-FT UCs (rare; an external UC tying roles across several
    // FTs) only short-circuit when every contributing FT
    // independently satisfies (a)+(b). Spanning UC over the full
    // tuple and deontic UC don't get key_roles in the first place,
    // so they always fall through to the legacy cross-scan.
    //
    ...
    if uc_storage_covered_for_all_groups(data, &span_groups) {
...
/// in `parse_forml2_stage2.rs`):
///
///   * `State_Machine_Definition_is_for_Noun` — SM ↔ Noun binding
///   * `Status_is_initial_in_State_Machine_Definition` — explicit initial
///   * `Status_is_rooted_in_State_Machine_Definition` — Pass-4
///     graph-derived candidates (forward-chain output of #760's rule)
///   * `Status_is_defined_in_State_Machine_Definition` — full status
///     set (Pass 1/2/2b output of #759's rule)
...
fn compile_state_machine_from_cells(
    ...
    let sm_names: Vec<String> = sm_for_noun.as_seq().map(|facts| {
        facts.iter().filter_map(|f| {
            let n = binding(f, "Noun")?;
            (n == noun_name).then(|| binding(f, "State Machine Definition")
                .map(|s| s.to_string()))?
        }).collect::<Vec<_>>()
    }).unwrap_or_default();

    // No SM for this noun — caller falls back to the JSON-blob path.
    let sm_name = sm_names.into_iter().next()?;

    // Helper: extract (Status, SM) pairs from a normalized SM cell that
    ...
    let statuses_for_sm = |cell_name: &str| -> Vec<String> {
        fetch_cell_seq(cell_name, cells).as_seq().map(|facts| {
            facts.iter().filter_map(|f| {
                ...
                (m == sm_name.as_str()).then(|| s.to_string())
            }).collect::<Vec<_>>()
        }).unwrap_or_default()
    };

    // -- Step 2: explicit initial status. -----------------------------
    let initial_decls = statuses_for_sm(
        "Status_is_initial_in_State_Machine_Definition");
    let explicit_initial: Option<String> = initial_decls.into_iter().next();

    // -- Step 3: full status set (Pass 1/2/2b). -----------------------
    let mut statuses: Vec<String> = statuses_for_sm(
        "Status_is_defined_in_State_Machine_Definition");
    ...
    let t_in_sm: Vec<String> = fetch_cell_seq(
        ...
        .as_seq().map(|facts| {
            facts.iter().filter_map(|f| {
                ...
                let m = binding(f, "State Machine Definition")?;
                (m == sm_name.as_str()).then(|| t.to_string())
            }).collect::<Vec<_>>()
        }).unwrap_or_default();
    ...
    let lookup = |pairs: &[(String, String)], key: &str| -> Option<String> {
        pairs.iter().find_map(|(k, v)| (k == key).then(|| v.clone()))
    };

    let mut transitions: Vec<TransitionDef> = t_in_sm.iter().filter_map(|t_name| {
        let from = lookup(&t_from_pairs, t_name)?;
        let to   = lookup(&t_to_pairs, t_name)?;
        let event = lookup(&t_event_pairs, t_name).unwrap_or_else(|| t_name.clone());
        // Backfill statuses encountered in transitions (parity with
        // the typed-struct Pass 3 fold at compile.rs:471-477).
        if !statuses.contains(&from) { statuses.push(from.clone()); }
        if !statuses.contains(&to)   { statuses.push(to.clone()); }
        Some(TransitionDef { from, to, event, guard: None })
    }).collect();

    // Stable order: the typed path uses a HashSet-driven iteration so
    // its order is non-deterministic. We sort transitions and statuses
    // so the cell-driven function emits a canonical Func that's
    // independent of cell-fact ordering.
    transitions.sort_by(|a, b| (&a.from, &a.event, &a.to).cmp(&(&b.from, &b.event, &b.to)));

    // -- Step 5: Pass-4 uniqueness gate. ------------------------------
    // When no explicit `Status is initial in SM` exists, fall back to
    // the rooted-set (Pass 4 candidates). Single rooted Status ⇒
    // initial; otherwise leave initial empty (matches Rust path's
    // "ambiguous" branch at compile.rs:502-504).
    let initial = match explicit_initial {
        Some(s) => s,
        None => {
            let rooted = statuses_for_sm(
                "Status_is_rooted_in_State_Machine_Definition");
            // De-dup defensively: the parser-shaped cell may carry the
            // same fact more than once if the upstream rule fired
            // multiple times.
            let mut unique: Vec<String> = Vec::new();
            for s in rooted {
                if !unique.contains(&s) { unique.push(s); }
            }
            if unique.len() == 1 { unique.into_iter().next().unwrap() }
            else { String::new() }
        }
    };
...
mod schema_tests {
    ...
    /// Single-role alethic UC whose key_roles covers the span: the
    /// UC compiler returns the no-op φ regardless of how the cell is
    /// shaped at compile time. Pre-1.0 contract: storage IS the UC
    /// enforcer for keyed cells. Callers that build state via legacy
    /// `cell_push` need to migrate to `cell_put_keyed`; the compiler
    /// no longer carries a Seq safety net.
    #[test]
    fn uc_compiler_emits_phi_regardless_of_cell_shape() {
        let compile_state = person_has_name_with_uc();
        // Don't install_map_cell — proves the predicate is purely
        // schema-level (key_roles covers UC) and doesn't depend on
        // runtime cell shape.
        let model = compile(&compile_state);
        let constraint = model.constraints.iter()
            .find(|c| c.id == "uc1")
        ...
        match &constraint.func {
            crate::ast::Func::Constant(o) => assert_eq!(*o, Object::phi()),
            other => panic!(
                "alethic UC with covering key_roles must compile to \
                 Constant(phi) even with no Map-shaped cell at compile \
                 time (pre-1.0: storage enforces, no Seq safety net); \
                 got {:?}", other),
        }
    }
    ...
    /// Post-task-820: UC violations on keyed cells surface through the
    /// storage layer (`ast::cell_put_keyed` returning `Err(KeyConflict)`),
    /// NOT through the constraint Func. This test pins the new contract:
    /// the Func is a no-op φ, and the duplicate-detection happens at
    /// write time.
    #[test]
    fn uc_violation_surfaces_via_cell_put_keyed_err() {
        let compile_state = person_has_name_with_uc();
        let model = compile(&compile_state);
        let constraint = &model.constraints[0];
        // The Func is the no-op (task-820): storage enforces.
        assert!(matches!(&constraint.func, crate::ast::Func::Constant(o) if *o == Object::phi()),
            "post-task-820: UC over key-covered FT must compile to Constant(phi); got {:?}",
            constraint.func);

        // Storage-layer enforcement: first write succeeds, second
        // write at the same scope key with a different non-key value
        // returns `Err(KeyConflict)`.
        let f1 = crate::ast::fact_from_pairs(&[("Person", "Alice"), ("Name", "Alice Smith")]);
        let f2 = crate::ast::fact_from_pairs(&[("Person", "Alice"), ("Name", "Alice Jones")]);
        let state = crate::ast::Object::phi();
        let state = crate::ast::cell_put_keyed("ft1", &["Person"], f1, &state)
            .expect("first put must succeed");
        let conflict = crate::ast::cell_put_keyed("ft1", &["Person"], f2, &state)
            .expect_err("duplicate Person 'Alice' with different Name must be a KeyConflict");
        assert_eq!(conflict.name, "ft1");
        assert_eq!(conflict.key, "Alice");
    }

    /// Distinct keys never collide; the storage layer accepts both.
    /// Mirror of the legacy `_no_violation_returns_phi` test, recast
    /// for the post-task-820 storage-as-constraint contract.
    #[test]
    fn distinct_keys_pass_through_cell_put_keyed_unviolated() {
        let compile_state = person_has_name_with_uc();
        let model = compile(&compile_state);
        // The compiled Func is still φ (storage enforces), and there's
        // nothing to surface — both writes succeed.
        assert!(matches!(&model.constraints[0].func, crate::ast::Func::Constant(o) if *o == Object::phi()));

        let f1 = crate::ast::fact_from_pairs(&[("Person", "Alice"), ("Name", "Alice Smith")]);
        let f2 = crate::ast::fact_from_pairs(&[("Person", "Bob"), ("Name", "Bob Jones")]);
        let state = crate::ast::Object::phi();
        let state = crate::ast::cell_put_keyed("ft1", &["Person"], f1, &state)
            .expect("Alice put must succeed");
        crate::ast::cell_put_keyed("ft1", &["Person"], f2, &state)
            .expect("Bob put must succeed — distinct key, no conflict");
    }
...
mod cell_driven_sm_tests {
    ...
    /// keep the test hermetic and avoid coupling to parser-side
    /// derivation/forward-chain timing — the function under test only
    /// reads the named cells, so populating them directly is sufficient.
    fn order_sm_cells_with_explicit_initial() -> Object {
        let mut cells: HashMap<String, Vec<Object>> = HashMap::new();
        ...
        for (t, from, to, ev) in &[
            ("place", "Draft",  "Placed",  "Order_was_placed"),
            ("ship",  "Placed", "Shipped", "Order_was_shipped"),
        ] {
    ...
    /// evaluate.rs.
    fn step(func: &ast::Func, state: &str, event: &str, defs: &Object) -> String {
        let input = Object::seq(vec![Object::atom(state), Object::atom(event)]);
        match ast::apply(func, &input, defs) {
            Object::Atom(s) => s,
            other => panic!("transition Func must return Atom; got {:?}", other),
        }
    }
    ...
    fn order_sm_cells_with_graph_derived_initial() -> Object {
        ...
        // No `Status is initial in SM` fact — the rooted cell is the
        // only signal.
        for s in &["Draft", "Placed", "Shipped"] {
            cells.entry("Status_is_defined_in_State_Machine_Definition".into()).or_default()
                .push(fact_from_pairs(&[
        ...
        // Pass-4 forward-chain output for a Draft -> Placed -> Shipped
        // chain: Draft is the unique source-never-target.
        cells.entry("Status_is_rooted_in_State_Machine_Definition".into()).or_default()
            .push(fact_from_pairs(&[
                ("Status", "Draft"),
                ("State Machine Definition", "OrderSM"),
            ]));
    ...
    #[test]
    fn graph_derived_initial_when_unique_source_never_target() {
        let cells = order_sm_cells_with_graph_derived_initial();
        let constraints: Vec<CompiledConstraint> = vec![];
        let cell_driven = compile_state_machine_from_cells(
            "Order", &cells, &constraints,
        ).expect("cell-driven path must return Some when SM-for-Noun exists");

        // Pass-4 uniqueness: rooted cardinality == 1 ⇒ promote to initial.
        assert_eq!(cell_driven.initial, "Draft",
            "Pass-4 uniqueness gate must promote the lone rooted Status \
             to initial when no explicit `is initial in` fact exists; \
             got initial={:?}, statuses={:?}",
            cell_driven.initial, cell_driven.statuses);

        // The transition Func must still classify the chain correctly.
        let defs = Object::phi();
        assert_eq!(step(&cell_driven.func, "Draft",  "Order_was_placed",  &defs), "Placed");
        assert_eq!(step(&cell_driven.func, "Placed", "Order_was_shipped", &defs), "Shipped");
    }

    /// Multiple rooted Statuses ⇒ ambiguous; the cell-driven path
    /// must leave `initial` empty (matches the typed path's behavior
    /// at compile.rs:5169).
    #[test]
    fn graph_derived_initial_left_empty_when_rooted_set_is_ambiguous() {
        let mut cells: HashMap<String, Vec<Object>> = HashMap::new();
        cells.entry("State_Machine_Definition_is_for_Noun".into()).or_default()
            .push(fact_from_pairs(&[
                ("State Machine Definition", "OrderSM"),
        ...
        // Two rooted Statuses → ambiguous → initial stays empty.
        ...
        assert!(cell_driven.initial.is_empty(),
            "ambiguous rooted set must leave initial empty; got {:?}",
            cell_driven.initial);
    }

    /// No SM-for-Noun fact ⇒ the cell-driven path returns None so the
    ...
    fn returns_none_when_no_sm_for_noun_fact() {
        ...
        assert!(result.is_none(),
            "cell-driven path must return None when no SM-for-Noun \
             fact binds the requested noun; got {:?}", result.map(|c| c.noun_name));
    }

    /// SM-for-Noun fact exists but no transitions ⇒ degenerate
    /// CompiledStateMachine whose transition Func is `Selector(1)`
    /// (returns current state). No panics on missing cells.
    #[test]
    fn empty_transitions_yields_degenerate_compiled_sm() {
        let mut cells: HashMap<String, Vec<Object>> = HashMap::new();
        cells.entry("State_Machine_Definition_is_for_Noun".into()).or_default()
            .push(fact_from_pairs(&[
                ("State Machine Definition", "OrderSM"),
        ...
        // No transitions / no statuses / no initial — everything degenerate.
        assert!(cell_driven.transition_table.is_empty());
        assert!(cell_driven.statuses.is_empty());
        assert!(cell_driven.initial.is_empty());

        // Selector(1) returns the current state for any input.
        let defs = Object::phi();
