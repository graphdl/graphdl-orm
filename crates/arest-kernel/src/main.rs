...
// (including the future #431 boot UI launcher) but are NOT yet wired
// into `entry_uefi.rs`'s boot flow — that's #431's territory.
//
// `pub` so the boot launcher track can `crate::ui_apps::hateoas::
// build_app()` without an inner-module gate. Available on every arch
