...
#define setup_common_bufsiz() ((void)0)\n";
    ...
    // Strip a copy for embedding: the kernel bakes the ELF into its own
    // image (`busybox_assets.rs` → `seed_busybox_file_cells`), and the
    // ~60% of the file that is host-side debug_info is dead weight inside
    // the population. The debug build at busybox.elf stays for inspection.
    let busybox_stripped = out_dir.join("busybox.stripped.elf");
    let objcopy = if cfg!(target_os = "windows") {
        "llvm-objcopy.exe" // same CreateProcess .exe quirk as ld.lld above
    } else {
        "llvm-objcopy"
    };
    let st = std::process::Command::new(objcopy)
        .arg("--strip-all")
        .arg(&busybox_elf)
        .arg(&busybox_stripped)
        .status()
        .map_err(|e| format!("spawn {} for busybox strip: {}", objcopy, e))?;
    if !st.success() {
        return Err(format!("busybox strip failed: {}", st));
    }
    // Emit the include_bytes! shim the kernel's `busybox_bin` module
    // includes — same canonical-absolute-path pattern as doom_assets.rs.
    let abs = abs_include_path(&busybox_stripped);
    fs::write(
        out_dir.join("busybox_assets.rs"),
        format!(
            "// Generated by build.rs — do not edit.\n\
             pub static BUSYBOX_ELF: &[u8] = include_bytes!(\"{}\");\n",
            abs,
        ),
    )
    .map_err(|e| format!("write busybox_assets.rs: {}", e))?;
    let stripped_len = fs::metadata(&busybox_stripped).map(|m| m.len()).unwrap_or(0);

    println!("cargo:rustc-link-search=native={}", out_dir.display());
    println!("cargo:rustc-cfg=busybox_built");
    println!(
        "cargo:warning=busybox: linked static ELF ({} bytes, {} stripped, {} sources, {} applets) at {}",
        elf_len,
        stripped_len,
        sources.len(),
        num_applets,
