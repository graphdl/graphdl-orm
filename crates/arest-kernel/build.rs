...
fn main() {
    ...
    // #629: server profile boots HATEOAS-only with no SPA — skip the
    // UI bundle bake entirely so a ~MB of JS / CSS / fonts doesn't
    // ride along in the .efi image. Re-emitted as an empty
    // `UI_ASSETS` table so `assets::lookup` cleanly returns `None`
    // and the http handler falls through to `system::dispatch` for
    // every non-`/arest/*` path.
    let server_profile = env::var_os("CARGO_FEATURE_SERVER").is_some();
    if server_profile {
        println!("cargo:warning=server profile: skipping ui.do bundle bake (#629)");
    } else if dist.is_dir() {
