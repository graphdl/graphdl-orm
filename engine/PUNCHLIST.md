...
8. GENERATORS (docs/07-generators.md). The old engine generates, per
   ...
   pyarest status: PORTED (2026-07-08). The SQLite DDL projection
   exists (ddl module, the GraphDL contract); dsl: ships always;
   the opt-in family xsd/owl/edm/html/dtd/wsdl/xforms/plix/nav is
   generated by generator_cells from one canon classification per
   noun (system:ev_cols + system:sqlname; NORMA's XML/OIALto*
   transforms at Repos/NORMA/XML are the format oracle). The verb
   cells (resolve:, create:, update:, list:, get:, transition:) are
   the runtime surface itself, not generators, and exist natively.
