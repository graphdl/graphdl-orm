...
to no host (Samuel, 2026-07-07): each platform consumes the SAME BYTES through
its own native front-end — Python `exec`s them (canon.py's read/load with the
vocabulary bound), rustc tokenizes them through `include!`, MSBuild
`ReadAllText`s them into the C# varargs wrap under `obj/` at build time, and
the Java wrap is regenerated by `gen_canon.py` (queued for retirement in favor
of booting the defs from the store, spec v3 — a host is a store loader and a
mu). Each platform defines a tiny vocabulary; the lambda bound determines the
implementation. No JSON shims, no bespoke parsers, no intermediate trees.
