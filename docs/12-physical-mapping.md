...
A single registry per scope, with all entity writes flowing through it, would violate Definition 2: two writers on disjoint entities in the same domain would contend on the scope's single lock. Per-domain registries still exist for genuinely scope-level concerns (population index, schema cache, snapshot storage) — those are appropriate to that granularity. What's per-cell is every entity write.
...
The OpenAPI surface advertises sort + order query parameters enumerated over each noun's RMAP columns, so a client can request `?sort=createdAt&order=desc` and the worker applies that ordering across the fanned-out reads. The sort is a post-fanout merge step — each DO has no awareness of sibling cells.
...
- **FPGA peers**: per-cell BRAM matches the per-DO model exactly — one memory unit per cell, one state machine per cell, all addressed by the same cell name.
